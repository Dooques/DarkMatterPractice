...
import darkmatter.ecs.components.PowerUpComponent
...
class PowerUpSystem(
    ...
    private var spawnPatterns = gdxArrayOf(
        SpawnPattern(type1 = PowerUpType.SPEED_1, type2 = PowerUpType.SPEED_2, type5 = PowerUpType.LIFE),
        SpawnPattern(type1 = PowerUpType.LIFE, type2 = PowerUpType.SHIELD, type5 = PowerUpType.SPEED_2)
    )
    ...
    override fun update(deltaTime: Float) {
        ...
        if (spawnTime <= 0 ) {
            ...
            spawnPowerUp(powerUpType, 1f * MathUtils.random(V_WIDTH - 1f), 16f)
        }
    }
    ...
    private fun spawnPowerUp(powerUpType: PowerUpType, x: Float, y: Float) {
        engine.entity {
            ...
            with<AnimationComponent> { type = powerUpType.animation }
    ...
    private fun collectPowerUp(player: Entity, powerUp: Entity) {
        ...
        powerUpCmp.type.also { powerUpType ->
            ...
            player[MoveComponent.mapper]?.let {
                it.speed.y += powerUpType.speedGain
            }
