...
class MoveSystem : IteratingSystem(
    ...
    override fun processEntity(entity: Entity, deltaTime: Float) {
        ...
        if (player != null) {
            ...
            entity[FacingComponent.mapper]?.let { facing ->
               movePlayer(transform, move, facing, deltaTime)
            }
        } else {
            /* Other Movement */
    ...
    private fun movePlayer(
        ...
        move.speed.x = when (facing.direction) {
            FacingDirection.LEFT -> min(
                0f,
                move.speed.x - HOR_ACCELERATION * deltaTime
            )
            FacingDirection.RIGHT -> max(
                0f,
                move.speed.x + HOR_ACCELERATION * deltaTime
            )
            else -> 0f
        }
        move.speed.x = MathUtils.clamp(
            move.speed.x,
            -MAX_HOR_SPEED,
            MAX_HOR_SPEED
        )
